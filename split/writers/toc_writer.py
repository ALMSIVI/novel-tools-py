...
class TocWriter(Writer):
    ...
    def __init__(self, args):
        '''
        Arguments:
        - out_filename (optional, str): Filename of the output toc. Defualt is toc.txt.
        - formats (dict[str, str]): Key is Type representations, and the value is the format string that can use any NovelData field.
        ...
        self.filename = os.path.join(out_dir, args.get('out_filename', 'toc.txt')) if os.path.isdir(out_dir) else out_dir
    ...
    def write(self, data: NovelData):
        ...
        title = data.format(self.formats[data.type])
