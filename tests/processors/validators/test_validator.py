...
def assert_data(data: NovelData, index: int, **kwargs):
...
def test_duplicate(overwrite_validator: StubValidator, no_overwrite_validator: StubValidator):
    ...
    assert_data(after1, 1)
    assert_data(after2, 1)

    before = NovelData('Duplicate First', Type.VOLUME_TITLE, 1)
    after1 = overwrite_validator.process(before)
    after2 = no_overwrite_validator.process(before)
    assert_data(after1, 2, original_index=1, error='Duplicate - expected: 2, actual: 1')
    assert_data(after2, 1, corrected_index=2, error='Duplicate - expected: 2, actual: 1')

    before = NovelData('Duplicate Second', Type.VOLUME_TITLE, 2)
    after1 = overwrite_validator.process(before)
    after2 = no_overwrite_validator.process(before)
    assert_data(after1, 3, original_index=2, error='Duplicate - expected: 3, actual: 2')
    assert_data(after2, 2, corrected_index=3, error='Duplicate - expected: 3, actual: 2')
...
def test_missing(overwrite_validator: StubValidator, no_overwrite_validator: StubValidator):
    ...
    after1 = overwrite_validator.process(before)
    after2 = no_overwrite_validator.process(before)
    assert_data(after1, 2, original_index=3, error='Missing - expected: 2, actual: 3')
    assert_data(after2, 3, corrected_index=2, error='Missing - expected: 2, actual: 3')

    before = NovelData('Skipping Third', Type.VOLUME_TITLE, 10)
    after1 = overwrite_validator.process(before)
    after2 = no_overwrite_validator.process(before)
    assert_data(after1, 3, original_index=10, error='Missing - expected: 3, actual: 10')
    assert_data(after2, 10, corrected_index=3, error='Missing - expected: 3, actual: 10')
...
def test_special(special_validator: StubValidator):
    ...
    assert_data(after, -1, special_index=1)

    before = NovelData('Duplicate First', Type.VOLUME_TITLE, -1, special_index=1)
    after = special_validator.process(before)
    assert_data(after, -1, special_index=2, original_index=1, error='Duplicate - expected: 2, actual: 1')

    before = NovelData('Missing Third', Type.VOLUME_TITLE, -1, special_index=4)
    after = special_validator.process(before)
    assert_data(after, -1, special_index=3, original_index=4, error='Missing - expected: 3, actual: 4')
...
def test_custom_special(custom_special_validator: StubValidator):
    ...
    assert_data(after, -1, special=1)

    before = NovelData('Duplicate First', Type.VOLUME_TITLE, -1, special=1)
    after = custom_special_validator.process(before)
    assert_data(after, -1, special=2, original_index=1, error='Duplicate - expected: 2, actual: 1')

    before = NovelData('Missing Third', Type.VOLUME_TITLE, -1, special=4)
    after = custom_special_validator.process(before)
    assert_data(after, -1, special=3, original_index=4, error='Missing - expected: 3, actual: 4')
